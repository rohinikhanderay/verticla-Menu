...
const EmployeeListing = (props) => {
  ...
  const removeUsers = (recId, uId) => {
    dispatch(removeEmp({ recId, uId })).then((res) => {
      getEmployeeAPI(pageNumber)
    })
  }
  return (
      ...
      <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
        <div className="container mx-auto px-6 py-8">
          ...
          <div className="mt-8 flex justify-end">
            {' '}
            <NavLink
              to={`/admin/organization/${props.match.params.id}/archiveEmployee`}
          ...
          {isShow ? (
            <ModelComponent
              ...
              orgId={props.match.params.id}
              type="ADMIN"
              employee={apiResponse.data && apiResponse.data}
              updateModel={modelFetch}
            />
          ) : (
            ''
          )}
                        ...
                        return (
                          <tr>
                            ...
                            <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 font-medium">
                              {item.organizationRole != 'admin' &&
                              item.organizationApproved.toString() == 'true' ? (
                                <span
                                  onClick={() => {
                                    if (item?.jobCount != 0) {
                                      showModel(item._id, item?.userRef)
                                    } else {
                                      removeUsers(item._id, item?.userRef)
                                    }
                                  }}
